...
CanvasRenderingContext2D.prototype.arcBetweenSegments = function(coordinates, radius) {
	radius = radius || 20;
	let XY = [];
...
<<<<<<< HEAD
	...
	if ((angle1 - angle2).abs() < Math.PI)
		this.drawArc(x, y, radius, [angle1, angle2].minE(), [angle1, angle2].maxE());
	else
		this.drawArc(x, y, radius, [angle1, angle2].maxE(), [angle1, angle2].minE());
=======
	...
	if ((diff - Math.PI / 2).abs() < 0.05) {
		...
		this.drawArc(x, y, radius, [angle1, angle2].minE(), (diffangle < Math.PI) ? [angle1, angle2].maxE() : [angle1, angle2].minE());
...
CanvasRenderingContext2D.prototype.drawLineAtAngle = function(x, y, angle, length) {
	...
	let coordinateX = x + angle.cos() * length;
	let coordinateY = y + angle.sin() * length;
	this.drawLine(x, y, coordinateX, coordinateY);
